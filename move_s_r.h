 ...
 * DESCRIPTION :
 * The following file provides the application with readable device data
 * and device structures to obtain signals from the Move S.
...
/* Main readable device structure */
struct move_s_r {
  move_s_r(pin_t bt_led, pin_t spk) :
  bt_led(new led(bt_led)), spk(new speaker(spk)) {
